...
class AccessProfile(Base, Timestamped):
    ...
    __tablename__='access_profiles'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); user_id: Mapped[str]=mapped_column(ForeignKey('users.id'),unique=True); role: Mapped[str]=mapped_column(String); max_active_leads: Mapped[int]=mapped_column(Integer,default=20); specialties: Mapped[list]=mapped_column(JSON,default=list); regions: Mapped[list]=mapped_column(JSON,default=list); supported_channels: Mapped[list]=mapped_column(JSON,default=list); timezone: Mapped[str]=mapped_column(String,default=''); working_hours: Mapped[dict]=mapped_column(JSON,default=dict); active: Mapped[bool]=mapped_column(Boolean,default=True)
class CampaignAssignment(Base, Timestamped):
    __tablename__='campaign_assignments'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_id: Mapped[str]=mapped_column(ForeignKey('campaigns.id')); representative_id: Mapped[str]=mapped_column(ForeignKey('users.id')); assigned_by_id: Mapped[str]=mapped_column(ForeignKey('users.id')); daily_send_limit: Mapped[int|None]=mapped_column(Integer,nullable=True); assigned_lead_limit: Mapped[int|None]=mapped_column(Integer,nullable=True); working_hours: Mapped[dict]=mapped_column(JSON,default=dict); routing_rule: Mapped[dict]=mapped_column(JSON,default=dict); active: Mapped[bool]=mapped_column(Boolean,default=True)
...
class DeliveryRecord(Base, Timestamped):
    """Immutable audit boundary for actual versus intended outbound recipients."""
    __tablename__='delivery_records'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_id: Mapped[str]=mapped_column(ForeignKey('campaigns.id')); prospect_id: Mapped[str]=mapped_column(ForeignKey('prospects.id')); conversation_id: Mapped[str|None]=mapped_column(ForeignKey('conversations.id'),nullable=True); approval_id: Mapped[str|None]=mapped_column(ForeignKey('approval_requests.id'),nullable=True); idempotency_key: Mapped[str]=mapped_column(String,unique=True); intended_recipient: Mapped[str]=mapped_column(String); actual_recipient: Mapped[str]=mapped_column(String); delivery_mode: Mapped[str]=mapped_column(String); channel: Mapped[str]=mapped_column(String); subject: Mapped[str]=mapped_column(String,default=''); body: Mapped[str]=mapped_column(Text); status: Mapped[str]=mapped_column(String,default='DELIVERED'); policy_decision: Mapped[dict]=mapped_column(JSON,default=dict); metadata_: Mapped[dict]=mapped_column('metadata',JSON,default=dict)
...
class ProspectResearch(Base, Timestamped):
    __tablename__='prospect_research'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_id: Mapped[str]=mapped_column(ForeignKey('campaigns.id')); prospect_id: Mapped[str]=mapped_column(ForeignKey('prospects.id')); status: Mapped[str]=mapped_column(String); research_summary: Mapped[str]=mapped_column(Text,default=''); person_research: Mapped[dict]=mapped_column(JSON,default=dict); company_research: Mapped[dict]=mapped_column(JSON,default=dict); icp_evidence: Mapped[list]=mapped_column(JSON,default=list); business_context: Mapped[list]=mapped_column(JSON,default=list); personalization_signals: Mapped[list]=mapped_column(JSON,default=list); sources: Mapped[list]=mapped_column(JSON,default=list); uncertainties: Mapped[list]=mapped_column(JSON,default=list); agent_run_id: Mapped[str]=mapped_column(ForeignKey('agent_runs.id'))
class ProspectFitment(Base, Timestamped):
    __tablename__='prospect_fitments'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_id: Mapped[str]=mapped_column(ForeignKey('campaigns.id')); prospect_id: Mapped[str]=mapped_column(ForeignKey('prospects.id')); organization_fit_score: Mapped[float]=mapped_column(Float); organization_fit_status: Mapped[str]=mapped_column(String); organization_criteria: Mapped[list]=mapped_column(JSON,default=list); contact_fit_score: Mapped[float]=mapped_column(Float); contact_fit_status: Mapped[str]=mapped_column(String); contact_criteria: Mapped[list]=mapped_column(JSON,default=list); overall_fit_score: Mapped[float]=mapped_column(Float); overall_fit_status: Mapped[str]=mapped_column(String); recommended_next_stage: Mapped[str]=mapped_column(String); key_fit_signals: Mapped[list]=mapped_column(JSON,default=list); key_risk_factors: Mapped[list]=mapped_column(JSON,default=list); uncertainties: Mapped[list]=mapped_column(JSON,default=list); research_id: Mapped[str]=mapped_column(ForeignKey('prospect_research.id')); engine_version: Mapped[str]=mapped_column(String,default='icp-fitment-v1')
...
class CampaignSetup(Base, Timestamped):
    """Wizard-only configuration; campaign remains the stable execution record."""
    __tablename__='campaign_setups'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_id: Mapped[str]=mapped_column(ForeignKey('campaigns.id'),unique=True); owner_id: Mapped[str|None]=mapped_column(ForeignKey('users.id'),nullable=True); revenue_range: Mapped[dict]=mapped_column(JSON,default=dict); funding_stage: Mapped[list]=mapped_column(JSON,default=list); technologies: Mapped[list]=mapped_column(JSON,default=list); exclusion_criteria: Mapped[list]=mapped_column(JSON,default=list); reference_profiles: Mapped[list]=mapped_column(JSON,default=list); custom_criteria: Mapped[dict]=mapped_column(JSON,default=dict); representative_settings: Mapped[dict]=mapped_column(JSON,default=dict)
class CampaignAgentConfig(Base, Timestamped):
    __tablename__='campaign_agent_configs'; id: Mapped[str]=mapped_column(String,primary_key=True,default=uid); campaign_agent_id: Mapped[str]=mapped_column(ForeignKey('campaign_agents.id'),unique=True); responsibilities: Mapped[list]=mapped_column(JSON,default=list); decision_thresholds: Mapped[dict]=mapped_column(JSON,default=dict); escalation_rules: Mapped[dict]=mapped_column(JSON,default=dict); prompt_version_id: Mapped[str|None]=mapped_column(ForeignKey('prompt_versions.id'),nullable=True)
class CampaignChannelSettings(Base, Timestamped):
