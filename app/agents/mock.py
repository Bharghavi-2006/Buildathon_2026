...
def personalize(prospect, campaign, facts, knowledge, channel):
    # Missing research is explicit: use only supplied prospect fields, never a made-up external fact.
    fact=facts[0].fact if facts else None
    context=fact or f'your work as {prospect.title} in {prospect.industry}'
    return PersonalizedOutreach(channel=channel,subject=f'Idea for {prospect.first_name}\'s team',body=f'Hi {prospect.first_name},\n\nI wanted to reach out regarding {context}. We help teams like yours turn GTM signals into focused outreach without losing human control. Would a 15-minute conversation next week be useful?\n\nBest,\nThe GTM team',personalization_facts=[fact] if fact else [],cta='15-minute conversation',reasoning=('Grounded in verified research' if fact else 'Reduced context: no research context available')+f' and {len(knowledge)} knowledge documents')
