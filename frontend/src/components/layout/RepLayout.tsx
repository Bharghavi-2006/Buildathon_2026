...
import { Bell, CheckSquare, HelpCircle, LayoutGrid, LineChart, LogOut, Megaphone, MessageSquare, Settings, ShieldAlert, ShieldCheck } from 'lucide-react';
  ...
  <aside className="w-60 bg-[#0c0e1f] border-r border-purple-500/10 flex flex-col justify-between p-4 h-screen sticky top-0 flex-shrink-0 z-20">
    <div>
      <div className="flex items-center gap-3 px-2 py-3 mb-6">
        <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-purple-600 to-indigo-700 flex items-center justify-center font-bold text-white shadow-md shadow-purple-900/30">A</div>
        <div>
          <div className="font-semibold text-white tracking-tight leading-tight">Autonomous SDR</div>
          <div className="text-[10px] uppercase tracking-wider text-slate-500">Rep Workspace</div>
        </div>
      </div>
      <nav className="space-y-1">
        ...
        <NavLink to="/rep/guardrails" className={navLinkClass}><ShieldCheck className="w-4 h-4" />Guardrails</NavLink>
      </nav>
    </div>
...
const RepTopBar: React.FC<{ used: number; limit: number }> = ({ used, limit }) => {
  ...
  return (
    <header className="h-16 border-b border-purple-500/10 bg-[#0a0c1b]/80 backdrop-blur-md px-6 flex items-center justify-end gap-3 sticky top-0 z-30">
