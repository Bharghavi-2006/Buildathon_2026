...
export const Step5ChannelsPrompts: React.FC<Step5ChannelsPromptsProps> = ({
  ...
  // Voice is never pausable from this wizard -- always on when any prospect has a phone number.
    ...
    if (channelsData.length > 0) {
      channelsData.forEach((c) => {
        ...
        if (name === 'email') {
          ...
          // Never restore a paused/disabled voice channel -- voice is always on here.
  ...
  return (
    <div className="space-y-8 max-w-4xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-start justify-between">
              ...
              {voiceAvailableCount === 0 ? (
                <span className="text-[10px] font-semibold text-slate-500">Unavailable</span>
              ) : (
                <span title="Voice is always live and cannot be paused" className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-amber-300">
                  <Lock className="w-3 h-3" />Always Live
                </span>
              )}
            </div>
      ...
      {/* SECTION 1.5: CAMPAIGN KNOWLEDGE BASE — source material for this campaign's RAG retrieval */}
        ...
        <div className="flex items-center gap-2.5 border-b border-purple-500/10 pb-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Paste text or drop a .txt/.md file. Each document is stored and scoped to this campaign only, and the Personalization agent retrieves the most relevant ones when drafting outreach.
            </p>
          </div>
        </div>
